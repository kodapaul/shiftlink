...
/**
 * Professional login form — presentational only.
 *
 * Owns the template + bindings; all state and submit logic lives in the
 * `useProfessionalLoginForm` composable, which in turn calls into the pure
 * service. Mirrors `StaffLoginForm.vue` so the two audiences feel like a
 * matching set.
 */
...
import { useProfessionalLoginForm } from '../composables/useProfessionalLoginForm'

const { values, errors, status, submit } = useProfessionalLoginForm()
</script>
